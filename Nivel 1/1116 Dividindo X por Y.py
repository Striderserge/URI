...
Este N será a quantidade de pares de valores inteiros (X e Y)
que serão lidos em seguida.

Saída
Para cada caso mostre o resultado da divisão com um dígito
...
for i in range(int(input())):
    entrada = input().split()
    if float(entrada[1]) == 0:
        print('divisao impossivel')
    else:
        print(f'{float(entrada[0]) / float(entrada[1]):.1f}')
